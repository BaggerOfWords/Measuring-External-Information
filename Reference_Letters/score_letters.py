...
def flatten_data(data_items, target_arg=None): 
    ...
    # --- Resolve Target Category (Replacing Reviewer logic) ---
    ...
    if target_arg is not None:
        ...
        if is_index:
            ...
            for item in data_items:
                ...
                if cat: all_categories.add(cat)
            
            sorted_categories = sorted(list(all_categories))
            if 0 <= target_idx < len(sorted_categories):
                target_category_name = sorted_categories[target_idx]
                print(f" >> Index {target_idx} selected: '{target_category_name}'")
            else:
                print(f" !! Index {target_idx} out of bounds. Exiting.")
                return []
        else:
            target_category_name = target_arg
            print(f" >> Filtering for category: '{target_category_name}'")

    print("Flattening data...")
    
    for i, item in enumerate(data_items):
        cat = item.get('category', 'unknown_category')
        
        # Filter by category if one was specified
        if target_category_name is not None:
            if cat != target_category_name:
                continue

        name = clean_string(item.get('name', 'Unknown Name'))
        occ = clean_string(item.get('occupation', 'Unknown Occupation'))
        target_text = clean_string(item.get('chatgpt_gen', ''))
        
        if len(target_text) < 50: continue 
        
        # Build dynamic Context (C)
        context_text = f"Please write a reference letter for {name}, an {occ}."
        
        # Create a unique ID for tracking
        safe_name = name.replace(" ", "_")
        unique_id = f"{cat}_{safe_name}_{i}"
    ...
    print(f" >> Loaded {len(flat_items)} generations.")
    return flat_items
...
class ContextualInfluenceScorer:
    def __init__(self, model, tokenizer, device, ratio, max_seq_len, hint_method, span_length, nb_repeats, tfidf_vectorizer=None, num_sentences=3):
        self.model = model
    ...
    def _get_token_logprobs(self, contexts, prompts, targets):
        ...
        # Adjust formatting (Changed 'Review:' to 'Letter:')
        ...
        for i, (ctx, t_text) in enumerate(zip(unified_contexts, target_texts)):
            if self.hint_method == "surprisal_spans":
                ...
                for _ in range(repeats):
                    ...
                    expanded_sparse_prompts.append(f"Hints: {sparse}")
            else:
                for _ in range(repeats):
...
def main():
    ...
    parser.add_argument("--target_category", type=str, default=None, help="Filter by category string or zero-indexed integer")
    args = parser.parse_args()

    print(f"--- Scoring Generations (Token-Level Deltas) ---")
    ...
    flat_data = flatten_data(raw_data, target_arg=args.target_category)

    tfidf_vectorizer = None
    if args.hint_method in ["keywords", "extractive"]:
        print("Fitting TF-IDF on the generated text corpus...")
    ...
    dataset = GenerationDataset(flat_data)
